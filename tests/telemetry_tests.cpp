...
class ReentrantShutdownExporter final : public IExporter {
public:
  [[nodiscard]] bool export_batch(std::span<const Record> records) override {
    exported.fetch_add(records.size(), std::memory_order_relaxed);
    if (on_export) {
      on_export();
    }
    return true;
  ...
  std::atomic<std::size_t> exported = 0;
};
...
[[nodiscard]] bool instruments_are_thread_safe() {
  ...
  const auto histogram_record =
      std::ranges::find_if(records, [](const Record &record) {
        return record.name() == "latency_ms";
      });
  const auto *data =
      histogram_record == records.end()
          ? nullptr
          : std::get_if<MetricData>(&histogram_record->payload());

  return check(counter.value() == workers * iterations, "concurrent counter") &&
         check(same_counter.value() == counter.value(),
               "shared named instrument") &&
...
[[nodiscard]] bool records_and_spans_work() {
  const auto event =
      Record::event("startup", {Attribute{.key = "node", .value = "one"}});
  Span span{"request"};
  ...
  return check(event.name() == "startup", "event name") &&
         check(std::holds_alternative<EventData>(event.payload()),
               "event payload") &&
...
[[nodiscard]] bool async_pipeline_drains() {
  ...
  for (std::size_t index = 0; index < 100; ++index) {
    if (!pipeline.publish(Record::event(
            "queued",
            {Attribute{.key = "index", .value = std::to_string(index)}}))) {
  ...
  const std::array rejected_batch{Record::event("late batch"),
                                  Record::event("late batch"),
                                  Record::event("late batch")};
  const bool rejected_all = !pipeline.publish(rejected_batch);
  const auto stats = pipeline.stats();
  return check(flushed, "async flush") &&
         check(exporter->size() == 117, "async drain") &&
...
[[nodiscard]] bool async_pipeline_accepts_concurrent_producers() {
  ...
  pipeline.shutdown();
  pipeline.shutdown();
  return check(accepted.load(std::memory_order_relaxed),
...
[[nodiscard]] bool async_pipeline_allows_worker_shutdown() {
  auto exporter = std::make_shared<ReentrantShutdownExporter>();
  using Policy = pipeline_policy::Async<8, 2>;
  vosp::TelemetryPipeline<Policy> pipeline{exporter};
  exporter->on_export = [&pipeline] { pipeline.shutdown(); };

  const bool accepted = pipeline.publish(Record::event("worker shutdown"));
  const bool flushed = pipeline.flush();
  pipeline.shutdown();
  const auto stats = pipeline.stats();
  return check(accepted, "worker shutdown acceptance") &&
         check(flushed, "worker shutdown drain") &&
         check(exporter->exported.load(std::memory_order_relaxed) == 1,
               "worker shutdown export") &&
         check(stats.accepted == 1 && stats.exported == 1,
...
int main() {
  return instruments_are_thread_safe() && registry_rejects_conflicts() &&
                 ...
                 async_pipeline_allows_worker_shutdown()
             ? 0
             : 1;
}
